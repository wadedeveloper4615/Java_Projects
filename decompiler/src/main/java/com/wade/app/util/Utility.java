...
public class Utility {
    ...
    /*
     * How many chars have been consumed during parsing in typeSignatureToString().
...
//                    continue;
//                }
//                // buf.append(ClassFileInstructions.getAccessName(i)).append(" ");
//                buf.append(access_flags.getName()).append(" ");
//            }
//        }
    ...
    /**
     * Convert (signed) byte to (unsigned) short value, i.e., all negative values
     * become positive.
     */
    ...
    public static String codeToString(final byte[] code, final ConstantPool constant_pool, final int index, final int length) throws ClassFormatException {
        return codeToString(code, constant_pool, index, length, true);
    }

    public static String codeToString(final byte[] code, final ConstantPool constant_pool, final int index, final int length, final boolean verbose) throws ClassFormatException {
    ...
    public static String codeToString(final ByteSequence bytes, final ConstantPool constant_pool) throws IOException, ClassFormatException {
        return codeToString(bytes, constant_pool, true);
    }

    public static String codeToString(final ByteSequence bytes, final ConstantPool constant_pool, final boolean verbose) throws IOException, ClassFormatException {
        ClassFileInstructions opcode = ClassFileInstructions.read((short) bytes.readUnsignedByte());
         ...
         * 4-byte-aligned
         */
        if ((opcode == ClassFileInstructions.TABLESWITCH) || (opcode == ClassFileInstructions.LOOKUPSWITCH)) {
    ...
    /**
     * Shorten long class name <em>str</em>, i.e., chop off the <em>prefix</em>, if
     * the class name starts with this string and the flag <em>chopit</em> is true.
    ...
    /**
     * Encode byte array it into Java identifier string, i.e., a string that only
     * contains the following characters: (a, ... z, A, ... Z, 0, ... 9, _, $). The
     * encoding algorithm itself is not too clever: if the current byte's ASCII
     * value already is a valid Java identifier part, leave it as it is. Otherwise
     * it writes the escape character($) followed by:
     *
     * <ul>
     * <li>the ASCII value as a hexadecimal string, if the value is not in the range
     * 200..247</li>
     * <li>a Java identifier char not used in a lowercase hexadecimal string, if the
     * value is in the range 200..247</li>
     * </ul>
     *
     * <p>
    ...
    /**
     * Fillup char with up to length characters with char `fill' and justify it left
     * or right.
     *
     ...
     * respect to the terms Descriptor and Signature. For the offical definitions
     * see:
     *
     * @see <a href=
     ...
     *
     *      In brief, a descriptor is a string representing the type of a field or
     *      method. Signatures are similar, but more complex. Signatures are used to
     *      encode declarations written in the Java programming language that use
     *      types outside the type system of the Java Virtual Machine. They are used
     *      to describe the type of any class, interface, constructor, method or
     *      field whose declaration uses type variables or parameterized types.
     *
     *      To parse a descriptor, call typeSignatureToString. To parse a signature,
     *      call signatureToString.
     *
     *      Note that if the signature string is a single, non-generic item, the
     *      call to signatureToString reduces to a call to typeSignatureToString.
     *      Also note, that if you only wish to parse the first item in a longer
     *      signature string, you should call typeSignatureToString directly.
     */
    ...
    /**
     * Converts argument list portion of method signature to string with all class
     * names compacted.
     *
    ...
    /**
     * Converts return type portion of method signature to string with all class
     * names compacted.
     *
     * @param signature Method signature
     ...
     * String name, final String access) { return methodSignatureToString(signature,
     * name, access, true); }
     *
     *
     * public static String methodSignatureToString(final String signature, final
     * String name, final String access, final boolean chopit) { return
     * methodSignatureToString(signature, name, access, chopit, null); }
     *
     * public static String methodSignatureToString(final String signature, final
     * String name, final String access, final boolean chopit, final
     * LocalVariableTable vars) throws ClassFormatException { final StringBuilder
     * buf = new StringBuilder("("); String type; int index; int var_index =
     * access.contains("static") ? 0 : 1; try { // Skip any type arguments to read
     * argument declarations between `(' and `)' index = signature.indexOf('(') + 1;
     * if (index <= 0) { throw new ClassFormatException("Invalid method signature: "
     * + signature); } while (signature.charAt(index) != ')') { final String
     * param_type = typeSignatureToString(signature.substring(index), chopit);
     * buf.append(param_type); if (vars != null) { final LocalVariable l =
     * vars.getLocalVariable(var_index, 0); if (l != null) {
     * buf.append(" ").append(l.getName()); } } else {
     * buf.append(" arg").append(var_index); } if ("double".equals(param_type) ||
     * "long".equals(param_type)) { var_index += 2; } else { var_index++; }
     * buf.append(", "); // corrected concurrent private static field acess index +=
     * unwrap(consumed_chars); // update position } index++; // update position //
     * Read return type after `)' type =
     * typeSignatureToString(signature.substring(index), chopit); } catch (final
     * StringIndexOutOfBoundsException e) { // Should never occur throw new
     * ClassFormatException("Invalid method signature: " + signature, e); } //
     * ignore any throws information in the signature if (buf.length() > 1) {
     * buf.setLength(buf.length() - 2); } buf.append(")"); return access +
     * ((access.length() > 0) ? " " : "") + // May be an empty string type + " " +
     * name + buf.toString(); }
     */
     ...
     */
//    public static short searchOpcode2(String name) {
...
//    }
    ...
    public static String signatureToString(final String signature) throws ClassFormatException {
        return signatureToString(signature, true);
    }

    public static String signatureToString(final String signature, final boolean chopit) throws ClassFormatException {
        String type = "";
        String typeParams = "";
        int index = 0;
    ...
    private static String typeParamTypesToString(final String signature, final boolean chopit) throws ClassFormatException {
    ...
    private static String typeParamTypeToString(final String signature, final boolean chopit) throws ClassFormatException {
    ...
    private static String typeSignaturesToString(final String signature, final boolean chopit, final char term) throws ClassFormatException {
    ...
    /**
     *
     * This method converts a type signature string into a Java type declaration
     * such as `String[]' and throws a `ClassFormatException' when the parsed type
     * is invalid.
     *
