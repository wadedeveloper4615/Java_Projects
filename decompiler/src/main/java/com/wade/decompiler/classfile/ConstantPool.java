 ...
 *
 * @see Constant
 * @see com.wade.decompiler.generic.ConstantPoolGen
 */
public class ConstantPool implements Cloneable, Node {
    ...
    public ConstantPool(final DataInput input) throws IOException, ClassFormatException {
        ...
        /*
         * constantPool[0] is unused by the compiler and may be used freely by the
         * implementation.
         */
        for (int i = 1; i < constant_pool_count; i++) {
            constantPool[i] = Constant.readConstant(input);
            /*
             * Quote from the JVM specification: "All eight byte constants take up two spots
             * in the constant pool. If this is the n'th byte in the constant pool, then the
    ...
    /**
     * Retrieves constant at `index' from constant pool and resolve it to a string
     * representation.
     *
    ...
    /**
     * Gets string from constant pool and bypass the indirection of `ConstantClass'
     * and `ConstantString' objects. I.e. these classes have an index field that
     * points to another entry of the constant pool of type `ConstantUtf8' which
    ...
    public String getConstantString(final int index, final byte tag) throws ClassFormatException {
        ...
        /*
         * This switch() is not that elegant, since the four classes have the same
         * contents, they just differ in the name of the index field variable. But we
         * want to stick to the JVM naming conventions closely though we could have
         * solved these more elegantly by using the same variable name or by
         * subclassing.
         */
        i = switch (tag) {
