...
public class MethodGen extends FieldGenOrMethodGen {
     ...
     * declares a local variable `$this' in slot 0. The actual code is contained in
     * the `il' parameter, which may further manipulated by the user. But he must
     * take care not to remove any instruction (handles) that are still referenced
     * from this object.
     * <p>
     * For example one may not add a local variable and later remove the
     * instructions it refers to without causing havoc. It is safe however if you
     * remove that local variable, too.
     *
     * @param access_flags access qualifiers
     * @param return_type  method type
     * @param argTypes     argument types
     * @param argNames     argument names (if this is null, default names will be
     *                     provided for them)
     * @param method_name  name of method
     * @param className    class name containing this method (may be null, if you
     *                     don't care)
     * @param il           instruction list associated with this method, may be null
     *                     only for abstract or native methods
    ...
    public MethodGen(final Method method, final String className, final ConstantPoolGen cp) {
        this(method.getAccessFlags(), Type.getReturnType(method.getSignature()), Type.getArgumentTypes(method.getSignature()), null /* may be overridden anyway */, method.getName(), className, ((method.getAccessFlags() & (Const.ACC_ABSTRACT | Const.ACC_NATIVE)) == 0) ? new InstructionList(getByteCodes(method)) : null, cp);
     ...
     * RuntimeParameterAnnotations, extracting their contents and storing them as
     * parameter annotations. There are two kinds of parameter annotation - visible
     * and invisible. Once they have been unpacked, these attributes are deleted.
     * (The annotations will be rebuilt as attributes when someone builds a Method
