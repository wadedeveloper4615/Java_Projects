...
public final class Code extends Attribute {
    ...
    Code(final int name_index, final int length, final DataInput file, final ConstantPool constant_pool) throws IOException {
        ...
        file.readFully(code);
        /*
         * Read exception table that contains all regions where an exception handler is
         * active, i.e., a try { ... } catch() block.
         */
        ...
        /*
         * Adjust length, because of setAttributes in this(), s.b. length is incorrect,
         * because it didn't take the internal attributes into account yet! Very subtle
         * bug, fixed in 3.1.1.
         */
     ...
     * @return String representation of code chunk.
     */
    public String toString2(final boolean verbose) {
        final StringBuilder buf = new StringBuilder(100); // CHECKSTYLE IGNORE MagicNumber
        buf.append(Utility.codeToString2(code, super.getConstantPool(), 0, -1, verbose));
        if (exceptionTable.length > 0) {
            buf.append("\nException handler(s) = \n").append("From\tTo\tHandler\tType\n");
            for (final CodeException exception : exceptionTable) {
                buf.append(exception.toString(super.getConstantPool(), verbose)).append("\n");
