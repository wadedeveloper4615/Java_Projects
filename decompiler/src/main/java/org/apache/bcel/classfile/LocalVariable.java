 ...
 * scope, name, signature and index on the method's frame. It is used both to
 * represent an element of the LocalVariableTable as well as an element of the
 * LocalVariableTypeTable. The nomenclature used here may be a bit confusing;
 * while the two items have the same layout in a class file, a
 * LocalVariableTable attribute contains a descriptor_index, not a
 * signatureIndex. The LocalVariableTypeTable attribute does have a
 * signatureIndex.
 * 
...
public final class LocalVariable implements Cloneable, Node, Constants {
     ...
     * @param index          Variable is `index'th local variable on the method's
     *                       frame
    ...
    /**
     * @param signatureIndex the index into the constant pool for the signature of
     *                       this variable
     */
