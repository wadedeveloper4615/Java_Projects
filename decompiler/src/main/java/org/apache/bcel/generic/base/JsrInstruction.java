...
public abstract class JsrInstruction extends BranchInstruction implements UnconditionalBranch, TypedInstruction, StackProducer {
     ...
     * JsrInstruction. <B>For this method to work, this JsrInstruction object must
     * not be shared between multiple InstructionHandle objects!</B> Formally, there
     * must not be InstructionHandle objects i, j where i != j and
     * i.getInstruction() == this == j.getInstruction().
     *
     * @return an InstructionHandle to the "next" instruction that will be executed
     *         when RETurned from a subroutine.
     */
