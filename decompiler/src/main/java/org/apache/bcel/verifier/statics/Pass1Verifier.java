...
public final class Pass1Verifier extends PassVerifier {
    ...
    /**
     * Pass-one verification basically means loading in a class file. The Java
     * Virtual Machine Specification is not too precise about what makes the
     * difference between passes one and two. The answer is that only pass one is
     * performed on a class file as long as its resolution is not requested; whereas
     * pass two and pass three are performed during the resolution process. Only
     * four constraints to be checked are explicitly stated by The Java Virtual
     ...
     * <LI>The constant pool must not contain any superficially unrecognizable
     * information.
     * </UL>
     ...
     * <UL>
     * <LI>there should only be constant_pool_count-1 many entries in the constant
     * pool.
     * <LI>all constant pool entries should have size the same as indicated by their
     * type tag.
     * <LI>there are exactly interfaces_count many entries in the interfaces array
     * of the class file.
     * <LI>there are exactly fields_count many entries in the fields array of the
     * class file.
     * <LI>there are exactly methods_count many entries in the methods array of the
     * class file.
     * <LI>there are exactly attributes_count many entries in the attributes array
     * of the class file, fields, methods, and code attribute.
     ...
     * content should be uncovered. For example, in an Exceptions attribute, the
     * actual number of exceptions as required by the number_of_exceptions field
     * might yeild an attribute size that doesn't match the attribute_length. Such
     * an anomaly should be detected.
     * <LI>all attributes should have proper length. In particular, under certain
     * context (e.g. while parsing method_info), recognizable attributes (e.g.
     * "Code" attribute) should have correct format (e.g. attribute_length is 2).
     * </UL>
     ...
     * <LI>All undocumented access flags are masked off before use. Strictly
     * speaking, this is not really a check.
     * <LI>The field this_class should point to a string that represents a legal
     * non-array class name, and this name should be the same as the class file
     * being loaded.
     * <LI>the field super_class should point to a string that represents a legal
     * non-array class name.
     * <LI>Because some of the above checks require cross referencing the constant
     * pool entries, guards are set up to make sure that the referenced entries are
     * of the right type and the indices are within the legal range (0 &lt; index
     * &lt; constant_pool_count).
     * </UL>
     * <LI>Extra checks done in pass 1:
     * <UL>
     * <LI>the constant values of static fields should have the same type as the
     * fields.
     * <LI>the number of words in a parameter list does not exceed 255 and
     * locals_max.
     * <LI>the name and signature of fields and methods are verified to be of legal
     * format.
     * </UL>
     * </UL>
     * (From the Paper <A HREF=
     * "http://www.cs.sfu.ca/people/GradStudents/pwfong/personal/JVM/pass1/"> The
     * Mysterious Pass One, first draft, September 2, 1997</A>.)
     *
     * <P>
     * However, most of this is done by parsing a class file or generating a class
     * file into BCEL's internal data structure. <B>Therefore, all that is really
     * done here is look up the class file from BCEL's repository.</B> This is also
     * motivated by the fact that some omitted things (like the check for extra
     * bytes at the end of the class file) are handy when actually using BCEL to
     * repair a class file (otherwise you would not be able to load it into BCEL).
     * </P>
     *
