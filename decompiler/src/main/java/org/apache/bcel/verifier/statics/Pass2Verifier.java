...
public final class Pass2Verifier extends PassVerifier implements Constants {
    ...
    private final class CPESSC_Visitor extends org.apache.bcel.classfile.EmptyVisitor {
        ...
        public void visitCode(final Code obj) {// vmspec2 4.7.3
            try {
                // No code attribute allowed for native or abstract methods: see
                // visitMethod(Method).
                ...
                // In JustIce, the check for correct offsets into the code array is delayed to
                // Pass 3a.
                ...
                for (final CodeException element : exc_table) {
                    ...
                    if (exc_index != 0) { // if 0, it catches all Throwables
                        ...
                        // We cannot safely trust any other "instanceof" mechanism. We need to
                        // transitively verify
                ...
                for (int a = 0; a < atts.length; a++) {
                    ...
                    if (atts[a] instanceof LocalVariableTable) { // checks conforming to vmspec2 4.7.9
                        ...
                        } // for all local variables localvariables[i] in the LocalVariableTable attribute
                          // atts[a] END
        ...
        public void visitConstantNameAndType(final ConstantNameAndType obj) {
            ...
            checkIndex(obj, obj.getNameIndex(), CONST_Utf8);
            // checkIndex(obj, obj.getDescriptorIndex(), CONST_Utf8); //inconsistently named
            // in BCEL, see below.
        ...
        public void visitConstantValue(final ConstantValue obj) {// vmspec2 4.7.2
            ...
            if (pred instanceof Field) { // ConstantValue attributes are quite senseless if the predecessor is not a
                                         // field.
                final Field f = (Field) pred;
                // Field constraints have been checked before -- so we are safe using their type
                // information.
        ...
        public void visitInnerClasses(final InnerClasses obj) {// vmspec2 4.7.5
            ...
            // exactly one InnerClasses attr per ClassFile if some inner class is refernced:
            // see visitJavaClass()
        ...
        public void visitJavaClass(final JavaClass obj) {
            ...
            if (hasInnerClass && !foundInnerClasses) {
                ...
                // '"+tostring(obj)+
                // "') must have exactly one InnerClasses attribute if at least one Inner Class
                // is referenced (which is the case)."+
        ...
        public void visitMethod(final Method obj) {
            ...
            // Nearly forgot this! Funny return values are allowed, but a non-empty
            // arguments list makes a different method out of it!
            ...
            if (jc.isClass()) {
                ...
                if (name.equals(Const.CONSTRUCTOR_NAME)) {
                    // ..may have at most one of ACC_PRIVATE, ACC_PROTECTED, ACC_PUBLIC set: is
                    // checked above.
                    // ..may also have ACC_STRICT set, but none of the other flags in table 4.5
                    // (vmspec2, page 115)
                ...
                if (!name.equals(Const.STATIC_INITIALIZER_NAME)) {// vmspec2, p.116, 2nd paragraph
                    if (jc.getVersion().getMajor() >= Version.Version_1_8.getMajor()) {
    ...
    public VerificationResult do_verify() {
        try {
            ...
            if (vr1.equals(VerificationResult.VR_OK)) {
                ...
                // For every method, we could have information about the local variables out of
                // LocalVariableTable attributes of
                // the Code attributes.
