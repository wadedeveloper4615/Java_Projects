...
/**
 * A Visitor class testing for valid preconditions of JVM instructions. The
 * instance of this class will throw a StructuralCodeConstraintException
 * instance if an instruction is visitXXX()ed which has preconditions that are
 * not satisfied. TODO: Currently, the JVM's behavior concerning monitors
 * (MONITORENTER, MONITOREXIT) is not modeled in JustIce.
 *
...
public class InstConstraintVisitor extends EmptyVisitor {
     ...
     * semantices of the superinterfaces and superclasses provided. E.g. SWAP is a
     * StackConsumer, but DUP_X1 is not a StackProducer. Therefore, this method is
     * called by all StackProducer, StackConsumer, and StackInstruction instances
     * via their visitXXX() method. Unfortunately, as the superclasses and
     * superinterfaces overlap, some instructions cause this method to be called two
     * or three times. [TODO: Fix this.]
     *
     ...
     * InstConstraintVisitor that a constraint violation has occured. This is done
     * by throwing an instance of a StructuralCodeConstraintException.
     *
    ...
    /**
     * Assures the ReferenceType r is initialized (or Type.NULL). Formally, this
     * means (!(r instanceof UninitializedObjectType)), because there are no
     * uninitialized array types.
     *
     * @throws StructuralCodeConstraintException if the above constraint is not
     *                                           satisfied.
     */
    ...
    /**
     * Sets the ConstantPoolGen instance needed for constraint checking prior to
     * execution.
     */
    ...
    /**
     * This returns the single instance of the InstConstraintVisitor class. To
     * operate correctly, other values must have been set before actually using the
     * instance. Use this method for performance reasons.
     *
     * @see #setConstantPoolGen(ConstantPoolGen cpg)
     * @see #setMethodGen(MethodGen mg)
     */
    public void setFrame(final Frame f) { // TODO could be package-protected?
        ...
        // throw new AssertionViolatedException("Forgot to set important values
        // first.");
    }

    /**
     * Sets the MethodGen instance needed for constraint checking prior to
     * execution.
     */
    ...
    public void visitARETURN(final ARETURN o) {
        ...
        // " at the stack top is not assignment compatible with the return type
        // '"+mg.getReturnType()+"' of the method.");
        // }
    }
    ...
    public void visitATHROW(final ATHROW o) {
        try {
            // It's stated that 'objectref' must be of a ReferenceType --- but since
            // Throwable is
            // not derived from an ArrayType, it follows that 'objectref' must be of an
            // ObjectType or Type.NULL.
    ...
    public void visitCHECKCAST(final CHECKCAST o) {
        ...
        // the runtime constant pool of the
        // current class (�3.6), where the value of the index is (indexbyte1 << 8) |
        // indexbyte2. The runtime constant
        // pool item at the index must be a symbolic reference to a class, array, or
        // interface type.
    ...
    public void visitGETFIELD(final GETFIELD o) {
        try {
            ...
            if (f.isProtected()) {
                ...
                final ObjectType curr = ObjectType.getInstance(mg.getClassName().getName());
                ...
                if (classtype.equals(curr) || curr.subclassOf(classtype)) {
                    ...
                    if (!(objreftype.equals(curr) || objreftype.subclassOf(curr))) {
                        ...
                        // "+
                        // "and it's a member of the current class or a superclass of the current
                        // class."+
    ...
    public void visitINVOKEINTERFACE(final INVOKEINTERFACE o) {
        ...
        // ConstantInterfaceMethodref cimr = (ConstantInterfaceMethodref)
        // (cpg.getConstant(o.getIndex()));
        ...
        for (int i = nargs - 1; i >= 0; i--) {
            ...
            if (!fromStack.equals(fromDesc)) {
                if (fromStack instanceof ReferenceType && fromDesc instanceof ReferenceType) {
                    ...
                    // TODO: This can only be checked when using Staerk-et-al's "set of object
                    // types"
        ...
        // TODO: This can only be checked if we're using Staerk-et-al's "set of object
        // types"
            ...
            for (int i = nargs - 1; i >= 0; i--) {
                ...
                if (!fromStack.equals(fromDesc)) {
                    if (fromStack instanceof ReferenceType && fromDesc instanceof ReferenceType) {
                        ...
                        // TODO: This can only be checked using Staerk-et-al's "set of object types",
                        // not
            ...
            for (int i = nargs - 1; i >= 0; i--) {
                ...
                if (!fromStack.equals(fromDesc)) {
                    if (fromStack instanceof ReferenceType && fromDesc instanceof ReferenceType) {
                        final ReferenceType rFromStack = (ReferenceType) fromStack;
                        final ReferenceType rFromDesc = (ReferenceType) fromDesc;
                        // TODO: This check can possibly only be done using Staerk-et-al's "set of
                        // object types"
                        // instead of a "wider cast object type" created during verification.
                        if (!rFromStack.isAssignmentCompatibleWith(rFromDesc)) {
                            constraintViolated(o, "Expecting a '" + fromDesc + "' but found a '" + fromStack + "' on the stack (which is not assignment compatible).");
            ...
            for (int i = nargs - 1; i >= 0; i--) {
                ...
                if (!fromStack.equals(fromDesc)) {
                    if (fromStack instanceof ReferenceType && fromDesc instanceof ReferenceType) {
                        final ReferenceType rFromStack = (ReferenceType) fromStack;
                        final ReferenceType rFromDesc = (ReferenceType) fromDesc;
                        // TODO: This can possibly only be checked when using Staerk-et-al's "set of
                        // object types" instead
    ...
    public void visitLoadInstruction(final LoadInstruction o) {
        // visitLocalVariableInstruction(o) is called before, because it is more
        // generic.
    ...
    public void visitPUTFIELD(final PUTFIELD o) {
        try {
            ...
            if (t instanceof ReferenceType) {
                ...
                // TODO: This can possibly only be checked using Staerk-et-al's "set-of-object
                // types", not
                // using "wider cast object types" created during verification.
                // Comment it out if you encounter problems. See also the analogon at
                // visitPUTSTATIC.
    ...
    public void visitReturnInstruction(final ReturnInstruction o) {
        ...
        if (o instanceof ARETURN) {
            ...
            // TODO: This can only be checked if using Staerk-et-al's "set of object types"
            // instead of a
            // "wider cast object type" created during verification.
            // if (! (objectref.isAssignmentCompatibleWith(mg.getType())) ) {
            // constraintViolated(o, "Type on stack top which should be returned is a
            // '"+stack().peek()+
            // "' which is not assignment compatible with the return type of this method,
            // '"+mg.getType()+"'.");
            // }
        } else {
    ...
    public void visitStoreInstruction(final StoreInstruction o) {
        ...
        if (stack().isEmpty()) { // Don't bother about 1 or 2 stack slots used. This check is implicitly done
                                 // below while type checking.
