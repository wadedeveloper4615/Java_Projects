...
/**
 * Instances of this class contain information about the subroutines found in a
 * code array of a method. This implementation considers the top-level (the
 * instructions reachable without a JSR or JSR_W starting off from the first
 * instruction in a code array of a method) being a special subroutine; see
 * getTopLevel() for that. Please note that the definition of subroutines in the
 * Java Virtual Machine Specification, Second Edition is somewhat incomplete.
 * Therefore, JustIce uses an own, more rigid notion. Basically, a subroutine is
 * a piece of code that starts at the target of a JSR of JSR_W instruction and
 * ends at a corresponding RET instruction. Note also that the control flow of a
 * subroutine may be complex and non-linear; and that subroutines may be nested.
 * JustIce also mandates subroutines not to be protected by exception handling
 * code (for the sake of control flow predictability). To understand JustIce's
...
public class Subroutines {
    ...
    private class SubroutineImpl implements Subroutine {
        /**
         * UNSET, a symbol for an uninitialized localVariable field. This is used for
         * the "top-level" Subroutine; i.e. no subroutine.
         */
        private static final int UNSET = -1;

        /**
         * The Local Variable slot where the first instruction of this subroutine (an
         * ASTORE) stores the JsrInstruction's ReturnAddress in and the RET of this
         * subroutine operates on.
         */
        ...
        /**
         * Returns a String representation of this object, merely for debugging
         * purposes. (Internal) Warning: Verbosity on a problematic subroutine may cause
         * stack overflow errors due to recursive subSubs() calls. Don't use this, then.
         */
        @Override
        ...
        /**
         * Sets the leaving RET instruction. Must be invoked after all instructions are
         * added. Must not be invoked for top-level 'subroutine'.
         */
        void setLeavingRET() {
        ...
        /*
         * Adds an instruction to this subroutine. All instructions must have been added
         * before invoking setLeavingRET().
         * 
         * @see #setLeavingRET
         */
        ...
        /*
         * Sets the local variable slot the ASTORE that is targeted by the
         * JsrInstructions of this subroutine operates on. This subroutine's RET
         * operates on that same local variable slot, of course.
         */
     ...
     * leader of the subroutine. Elements: SubroutineImpl objects.
     */
    private final Map<InstructionHandle, Subroutine> subroutines = new HashMap<>();

    /**
     * This is referring to a special subroutine, namely the top level. This is not
     * really a subroutine but we use it to distinguish between top level
     * instructions and unreachable instructions.
     */
    // CHECKSTYLE:OFF
     ...
     * Constructor.
     * 
     * @param mg A MethodGen object representing method to create the Subroutine
     *           objects of. Assumes that JustIce strict checks are needed.
     */
     ...
     * Constructor.
     * 
     * @param mg                 A MethodGen object representing method to create
     *                           the Subroutine objects of.
    ...
    public Subroutines(final MethodGen mg, final boolean enableJustIceCheck) {
        ...
        for (final InstructionHandle actual : sub_leaders) {
            ...
            /*
             * BFS ALGORITHM MODIFICATION: Start out with multiple "root" nodes, as
             * exception handlers are starting points of top-level code, too. [why
    ...
    /**
     * This (recursive) utility method makes sure that no subroutine is calling a
     * subroutine that uses the same local variable for the RET as themselves
     * (recursively). This includes that subroutines may not call themselves
     * recursively, even not through intermediate calls to other subroutines.
     *
     * @throws StructuralCodeConstraintException if the above constraint is not
     *                                           satisfied.
     */
    ...
    /**
     * Returns the Subroutine object associated with the given leader (that is, the
     * first instruction of the subroutine). You must not use this to get the
     * top-level instructions modeled as a Subroutine object.
     *
     * @see #getTopLevel()
     */
    ...
    /**
     * Returns the subroutine object associated with the given instruction. This is
     * a costly operation, you should consider using
     * getSubroutine(InstructionHandle). Returns 'null' if the given
     * InstructionHandle lies in so-called 'dead code', i.e. code that can never be
     * executed.
     *
    ...
    /**
     * For easy handling, the piece of code that is <B>not</B> a subroutine, the
     * top-level, is also modeled as a Subroutine object. It is a special Subroutine
     * object where <B>you must not invoke getEnteringJsrInstructions() or
     * getLeavingRET()</B>.
     *
